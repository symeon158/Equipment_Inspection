...
image = Image.open("heights.jpg")
st.title("🪜Equipment Working At Heights")
st.image(image)
...
equipments = ["",  "Safety harnesses",  "Lanyards",  "Fall arrest systems",  "Safety nets",  "Scaffolding",  "Aerial work platforms",  "Rope access equipment",  "Helmets",  "Safety shoe",  "Guardrails"]
...
col2.write("Click the button to scan a QR code.")
if equipment != "":
...
if situation == "Broken Down" :
    ...
    if st.session_state.warning_displayed or not st.session_state.get("comments", ""):
        st.warning("Please provide comments for breakdown.")
    else:
